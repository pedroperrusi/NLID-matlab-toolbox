...
% Although it is quite noisy, the marginal kernel appears to be proportional
% to the first-order kernel.  Thus, the kernels appear to be consistent with
% the hypothesis that the underlying system is a LNL cascade (which includes
% both Wiener and Hammerstein cascades as special cases).
...
% The (scaled) diagonal of the second-order kernel appears
...
% Notice that the first and second off-diagonals (green and red) appear
% to contain only noise, whereas the diagonal (blue) contains a significant
% peak at about 0.005 seconds lag. To be absolutely sure, plot the kernel
% in 3D, and look along the diagonal.
...
% Since there is no singificant structure anwhere other than on the
% diagonal, the kernels are consistent with
% the hypothesis that the system is a Hammerstein cascade.
...
% Since the primary singular vector is clearly not proportional to the 
% first-order kernel, the underlying system cannot be a Wiener cascade.
