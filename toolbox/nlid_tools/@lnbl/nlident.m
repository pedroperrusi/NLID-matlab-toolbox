...
if isa(z,'nldat') || isa(z,'double')
    ...
    x=z(:,1);
    y=z(:,2);
    hlen = get_nl(bl,'NLags');
    mode = get_nl(bl,'mode');
    set(i,'NLags',hlen,'mode',mode); % Debug: needs regular set
    ...
    bl.nlm{1,2}=m_est  ;
    set(bl,'NLags',get_nl(h_est,'NLags'));
    
    
    switch get_nl(bl,'method')
        case 'bussgang'
            set_nl(bl,'Comment','LN model identified using Busgang''s theorm');
        case 'hk'
            bl = hk_ident(bl,z);
        case 'phk'
            bl = phk_ident(bl,z);
