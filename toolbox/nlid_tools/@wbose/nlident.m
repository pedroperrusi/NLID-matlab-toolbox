...
if isa(z,'nldat') || isa(z,'double')
    ...
    if isa(z,'nldat')
        ...
        bank = get_nl(wb,'elements');
        f1 = bank{1};
        Ts = get_nl(f1,'domainincr');
        z = nldat(z,'domainincr',Ts);
    end
    
    method = lower(get_nl(wb,'method'));
    switch method
        case 'laguerre'
            wb = laguerre_wb(wb,z);
        case 'pdm'
            vs = vseries;
            IDmethod = get_nl(wb,'id-method');
            set(vs,'method',IDmethod,'NLags',get_nl(wb,'NLags'),'ordermax',2);
            vs = nlident(vs,z);
            wb = pdm(vs,wb);
            wb = poly_wb(wb,z);
        case 'poly'
...
kernel_method = get_nl(z,'method');
if strcmpi(kernel_method,'laguerre')
    max_modes = get_nl(z,'NumFilt');
    ss = ss(1:max_modes);
else
    max_modes = hlen;
end
...
mode = lower(get_nl(wb,'mode'));

switch mode
    case 'fixed'
        num_filts = get_nl(wb,'NFilt');
    case 'auto'
