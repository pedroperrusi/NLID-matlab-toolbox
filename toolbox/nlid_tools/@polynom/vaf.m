...
% VAF function for NLM polynomial models
%
% vf = vaf(model,data)
%
% where model is any model of class polynom
...
if nargin == 1
  error('I/O data must be supplied if first argument is of class POLYNOM');
end
...
if nargin == 2
  ...
  elseif m > nin+1
    error('To test MIMO systems, vaf(poly,inputs,outputs)');
  end
  u = nldat(ddata(:,1:nin));
  y = nldat(ddata(:,nin+1));
  if Ts > 0
