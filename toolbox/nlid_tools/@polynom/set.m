    ...
    if j>0
        ...
        switch Property
            case 'coef'
                ...
                if ~ismatrix(Value) | ~isreal(Value) | ~isfinite(Value) %#ok<OR2>, single operators necessary
            ...
            case 'ninputs'
                sys.NInputs = Value;
            case 'order'
                if ~ismatrix(Value) || ~isreal(Value) || ~isfinite(Value)
            ...
            case 'std'
                sys.Std = Value;
            case 'type'
                if ~any(strcmpi(Value,{ 'hermite' 'power' 'tcheb'}))
