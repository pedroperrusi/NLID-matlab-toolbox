...
% Copyright 1991-2003, Robert E Kearney, James P Trainor and Mireille Lortie
...
% A = X' - ones(size(X,2),1)*mean(X');  %05/12/05 HIG: commented this section out
% X = A';
% clear A

% Remove the ensemble average from the output. 
% A = Y' - ones(size(Y,2),1)*mean(Y'); %05/12/05 HIG: commented this section out
% Y = A';
% clear A
...
elseif (strcmp(approach,'pseudo'))
   ...
   for i = H_domain_start:H_domain_end
      ...
      [minMDL, r] = min(MDL);
      sing_vectors(i-H_domain_start+1) = r;
      %disp(['Using ' num2str(r) ' of ' num2str(length(MDL)) 'singular values']); 
