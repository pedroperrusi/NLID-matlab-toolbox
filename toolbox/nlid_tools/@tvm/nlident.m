...
Mout=Min;
ModelType= get_nl(Min, 'Model_Type');
%
...
switch ModelType
    case 'polynom'
        ...
        % TV IRF
        %
        di=get_nl(z,'DomainIncr');set(Mout,'domainincr',di);
        dn=get_nl(z,'DomainName');set(Mout,'domainname',dn);
        ds=get_nl(z,'DomainStart');set(Mout,'domainstart',ds);
        dv=get_nl(z,'DomainValues');set(Mout,'domainvalues',dv);
    ...
    case 'nlbl'
        ...
        p=polynom;
        set(p,'Type','power','OrderMax',OrderMax,'Order',OrderMax);
        iout=irf;
