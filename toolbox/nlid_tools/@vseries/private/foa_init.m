...
phi = cor;
set(phi,'Order',1,'Bias','biased','NSides',1,'NLags',hlen,'type','correl');
mu_y = mean(y);
G(1) = mu_y;
...
if Qmax > 1
    phi3u = double(nlident(phi,u,'order',2));
    phi4u = double(nlident(phi,u,'order',3));
    phi4u = phi4u(:);
    phiuuy = double(nlident(phi,[u y],'order',2));
    m = hlen+2;
    for i = 1:hlen
        m1 = hlen - i;
