...
hlen = get_nl(vs,'NLags');
Qmax = get_nl(vs,'OrderMax');
kernels = get_nl(vs,'elements');
vk0 = kernels{1};
...
h0 = A(1);
kernels{1} = vkern(vk0,'order',0,'data',h0);
if Qmax > 0
    h1 = A(2:hlen+1);
    kernels{2} = vkern(vk0,'order',1,'data',h1/Ts);
end
if Qmax > 1
    m = hlen + 1;
    for i = 1:hlen
    ...
    kernels{3} = vkern(vk0,'order',2,'data',h2/(Ts^2));
end
