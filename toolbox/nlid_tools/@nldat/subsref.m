...
if strcmp (S.type, '()')
    ...
    set(datout,'data',dout); % Must be regular SET!
...
elseif strcmp(S.type,'.')
    switch lower(S.subs)
        case  'channames'
            datout = datin.ChanNames;
        case 'chanunits'
            datout = datin.ChanUnits;
        case 'domainincr'
            datout=datin.DomainIncr;
        case 'domainname'
            datout=datin.DomainName;
        case 'domainstart'
            datout=datin.DomainStart;
        case 'data'
            datout=datin.Data;
        otherwise
            error(['nldat susref error. Field not defined' S.type])
    end
