...
for i=1:size(Props,1)
    ...
    if isa(val,'cell') && isequal(size(val),[1 1])
        val1 = val{1};
        if (ischar(val1) || isa(val1,'double')) && ismatrix(val1) && size(val1,1)<=1
            val = val1;
            cellflag = 1;
        end
    end
    ...
    elseif isa(val,'double') && ismatrix(val) && ...
    ...
    elseif isa(val,'param')
        n=size(val);
        names=get_nl(val,'name');
        val_str=[pad pad names{1} ':' num2str(getvalue(val,1)) ];
        for j=2:n
            val_str=char(val_str, [pad pad names{j} ':' num2str(getvalue(val,j)) ]);
        end
    else
